...
public class Snake {
	...
	//head collided with wall:true;else:false
	public boolean collisionWall(int WIDTH,int HEIGHT) {
		return (this.headLocation.getX()<0)||
				(this.headLocation.getX()>=WIDTH)||
				(this.headLocation.getY()<0)||
				(this.headLocation.getY()>=HEIGHT);
	}
	
	//head collided with body:true; else:false
	public boolean collisionBody() {
		return this.body.passing(this.headLocation);
	}
	
	//to set direction of the snake
	public Snake setDirection(IDirection d) {
		
		return new Snake (this.headLocation, this.body, d);
	}
